	...
	public static RBST orderTeams(int N) {
		...
		for (int i = 1; i < N; i++) {	
			...
			else 
			{
				...
				int end = O.getSize(); //get the final root in the "generation"
				int first = 1; //set for determining the first root
				
				while(first < end - 1) //while loop through the entire generation
				{
					int cent = ((first + end) / 2);
					if (didXBeatY(i, O.select(cent).getTeam()) != true)
					{
						first = cent;
					}
					else
